...
ショット生成ページにも、個別の候補生成に加えて「未生成分の候補を順次生成」ボタンがあります。個別生成では正式採用済みの直前ショットだけを参照します。一括順次生成では、`shot_001` の候補を生成した直後に、その未採用候補を `shot_002` の視覚参照へ渡すように連鎖します。途中に正式採用済みショットがあれば、その画像を次の連鎖の起点にします。一括処理でも自動採用は行わず、構図崩れや人物の変形を確認してから個別に採用します。
