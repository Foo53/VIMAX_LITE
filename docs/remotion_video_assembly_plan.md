...
このドキュメントは、動画生成 API を使わずに、生成済みショット画像をつなげて、字幕と音声つきの動画へ発展させるための設計です。現在は字幕付き動画を生成でき、MVモードではアップロードしたBGMを組み込めます。SEと読み上げ音声は今後の拡張です。
...
- 画像を順番に表示する。
- 字幕を焼き込む。
- MVモードではアップロード済みBGMを重ねる。SEと読み上げ音声はまだ重ねない。
...
- BGMトラックを追加する（MVモードのアップロードBGMは実装済み）。
- ショット単位のSEを追加する。
- `TTSProvider` を追加する。
- `mock` では無音音声を作る。
- 実 provider は後から追加する。
...
現在の実装は、字幕付きの画像連結動画と、MVモードでのアップロードBGM再生までを対象にしています。

次に追加する予定の機能:

- BGM: MVモード以外の作品にも作品全体の音楽トラックを設定できるようにする。
